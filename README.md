...
4. Setup db server (optional, depends on your `.env`)
   1. The system uses PostgreSQL. It is highly recommended for performance and ARM64 compatibility (Apple Silicon).
   2. Go back into root folder.
   3. ```bash
      # (on Windows replace `pwd` with `pwd -W`)
      docker run --name main_gate_aplr_db \
       -e POSTGRES_PASSWORD='MyV€ryStr0ngP4ssW0rĐ' \
       -e POSTGRES_DB=lpdb \
       -p 5432:5432 \
       -v $(pwd)/db/data:/var/lib/postgresql/data \
       -d postgres:15-alpine
      ```
   4. To initialize the schema:
